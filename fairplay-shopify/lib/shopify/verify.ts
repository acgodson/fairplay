...
export async function verifyAuth(shop: string, online?: boolean) {
  ...
  if (online) {
    const onlineSession = sessions.find(
      (session: any) => session.isOnline === true,
    );
  ...
  const offlineSession = sessions.find(
    (session: any) => session.isOnline === false,
  );
...
export async function verifyRequest(req: Request, isOnline: boolean) {
  ...
  try {
    ...
    if (session.isOnline && session.expires) {
      const expiresTimestamp = session.expires as any;
      const expiresDate = new Date(expiresTimestamp._seconds * 1000);
